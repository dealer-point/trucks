...
|-- assets
    |-- template
        ...
        |-- js
        |-- stylesheets
|-- main *Модуль шаблонов и дополнительных компонентов приложения*
|-- core *Модуль функционального ядра приложения*
|-- companies *модуль, подключающий функционал компаний*
|-- libs
|-- interfaces
```

###  Написание и подключение модулей
Новый модуль добавляется путем создания в папке `frontend` директории с названием модуля. Данный модуль должен быть самостоятельным модулем angular. Внутри должен находиться файл `index.ts` - основная точка входа модуля. Модуль должен содержать отдельный файл `routes.ts` который регистрирует свои маршруты (states) для angular-ui-router. Если модуль содержит расширение карты, например, комппаний, то помимо маршрутов необходимо добавить пункты меню в специальный сервис `CompanyCardToolsService`. Данный сервис наследуется от базового сервиса `CardTools`, по этому если новый модуль будет содержать свою карту, например, контакта, то необходимо создать сервис для хранения списка меню, унаследовав его от глобального класса `CardTools`.
...
Модуль `core` содержит основное ядро приложения, т. е. в его состав должны входить компоненты, предоставляющие базовый функционал приложения, например пользователи, роли и т. п.

Модуль `companies` подключает к приложению функционал компаний. Данный модуль добавлеят списки компаний, детали, сервис по созданию компаний через модальное окно и т. д..
