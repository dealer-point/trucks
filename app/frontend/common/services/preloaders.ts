        ...
        $rootScope.content_preloader_show = (style?: string, container?: JQuery): void => {
            ...
            if (!$body.find('.content-preloader').length) {
                let isHighDensity: boolean = ((window.matchMedia &&
                    (window.matchMedia(`only screen and (min-resolution: 124dpi), only screen and
                                       (min-resolution: 1.3dppx), only
                                       screen and (min-resolution: 48.8dpcm`).matches ||
                        window.matchMedia(`only screen and (-webkit-min-device-pixel-ratio: 1.3),
                                          only screen and (-o-min-device-pixel-ratio: 2.6/2), only
                ...
                let preloaderContent: string = (typeof style !== 'undefined'
                                                && style === 'regular') ? `
                    <img
                        src="assets/img/spinners/spinner${imageDensity}.gif"
                        alt=""
                        width="32"
                        height="32" />`
                    : `
